...
snapFolder.add(snapConfig, 'enabled').onChange((v: boolean) => v ? dimSystem.snappingManager.enable() : dimSystem.snappingManager.disable());
snapFolder.add(snapConfig, 'vertices').onChange((v: boolean) => dimSystem.snappingManager.snapVertices = v);
snapFolder.add(snapConfig, 'edges').onChange((v: boolean) => dimSystem.snappingManager.snapEdges = v);
snapFolder.add(snapConfig, 'midpoints').onChange((v: boolean) => dimSystem.snappingManager.snapMidpoints = v);
snapFolder.add(snapConfig, 'centroids').onChange((v: boolean) => dimSystem.snappingManager.snapCentroids = v);

const styleFolder = gui.addFolder('Dimension Style');
const styleConfig = dimSystem.dimensionRenderer.style;
styleFolder.addColor(styleConfig, 'color').onChange(() => dimSystem.dimensionRenderer.setStyle({ color: styleConfig.color }));
styleFolder.add(styleConfig, 'scale', 0.1, 5).onChange(() => dimSystem.dimensionRenderer.setStyle({ scale: styleConfig.scale }));
styleFolder.add(styleConfig, 'depthTest').onChange(() => dimSystem.dimensionRenderer.setStyle({ depthTest: styleConfig.depthTest }));
styleFolder.add(styleConfig, 'units', ['m', 'mm', 'ft']).onChange(() => dimSystem.dimensionRenderer.setStyle({ units: styleConfig.units as any }));
styleFolder.addColor(styleConfig, 'textBgColor').name('Text BG').onChange(() => dimSystem.dimensionRenderer.setStyle({ textBgColor: styleConfig.textBgColor }));
...
modeFolder.add(modeConfig, 'mode', ['linear', 'angle', 'leader']).onChange((v: string) => {
...
function getSnappableObjects() {
  ...
  scene.traverse((child) => {
    ...
    if (child === dimensionsGroup || child === previewGroup || child === cplaneHelper) return;
    if (child.parent === dimensionsGroup || child.parent === previewGroup || child.parent === cplaneHelper) return;
